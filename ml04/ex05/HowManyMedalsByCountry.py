...
## Merge & Sum 
# https://stackoverflow.com/questions/19882641/sum-value-of-two-different-dictionaries-which-is-having-same-key 
...
def how_many_medals_by_country(df, country) : 
    ...
    good_country = df[df['Team'] == country]
    good_country.sort_values(by=['Year'])
    

    out_teams = good_country[good_country['Sport'].isin(team_sports) == False]
    only_teams = good_country[good_country.Sport.isin(team_sports)]
    only_teams = only_teams.drop_duplicates(['Year', 'Medal', 'Event'])

    all_teams = pd.concat([out_teams, only_teams])
    ret_dict = dict(enumerate(all_teams.Year.unique()))
    ret_dict = {v: k for k, v in ret_dict.items()}

    for key, value  in ret_dict.items() :
        ret_dict[key] = {
        'G' : sum((all_teams.Year == key) & (all_teams.Medal == 'Gold')),
        'S':  sum((all_teams.Year == key) & (all_teams.Medal == 'Silver')),
        'B':  sum((all_teams.Year == key) & (all_teams.Medal == 'Bronze'))}
    return ret_dict
    # merge & Sum two dict 
